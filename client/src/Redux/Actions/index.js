...
const paginationAndFilter = ({
   ...
   return async (dispatch) => {
      const paginationFilter = await axios(`http://localhost:3001/paginationAndFilter?quantityElementsByPage=${quantityElementsByPage}&page=${page}&dietFilter=${dietFilter}`)
...
const searchByName = (payload) => {
   return async (dispatch) => {
      const allDiets = await axios(`http://localhost:3001/recipes?name=${payload}`)
...
const getDetailRecipe = (payload) => {
   return async (dispatch) => {
      const detailRecipe = await axios(`http://localhost:3001/recipes/${payload}`)
