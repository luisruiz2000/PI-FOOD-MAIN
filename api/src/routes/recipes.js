...
router.get('/', async (req, res) => {
   ...
   try {
      if (name) {
         let recipeInfo = await recipesTotal.filter(recipe => recipe.name.toLowerCase().includes(name.toLowerCase()))
         recipeInfo.length ?
            res.status(200).json(recipeInfo) :
            res.status(404).json({ msg: 'No se encontraron resultados, Intenta buscar otra receta' })
      } else {
...
router.get('/:id', async (req, res) => {
   ...
   if (Number(idP)) {
      const urlId = await axios(`https://api.spoonacular.com/recipes/${idP}/information?apiKey=${APIKEY}`)
      res.json(urlId.data)
   } else {
...
router.post('/', async (req, res) => {
   let { name, summary, healthScore, instructions, createdInDb, diets } = req.body
   try {
